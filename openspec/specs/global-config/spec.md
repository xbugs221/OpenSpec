...
The system SHALL store global configuration in `~/.config/openspec/config.json` for user-level OpenSpec settings.

#### Scenario: Initial config creation
- **WHEN** no global config file exists
- **AND** the system saves global configuration
- **THEN** the system creates `~/.config/openspec/config.json`
...
- **WHEN** saving new OpenSpec settings into an existing config file
