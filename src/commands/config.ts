...
export function registerConfigCommand(program: Command): void {
  ...
  configCmd
    ...
    .action((key: string) => {
      if (isRemovedLegacyConfigKey(key)) {
        console.error(`Error: "${key}" is a removed legacy installation key and is no longer supported.`);
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .description('Removed legacy workflow-install configuration')
    .action(() => {
      console.error('Error: `openspec config profile` has been removed. OpenSpec no longer installs workflows as skills or prompts.');
      process.exitCode = 1;
    });
}
